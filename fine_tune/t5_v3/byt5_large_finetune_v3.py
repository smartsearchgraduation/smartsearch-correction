...
DOWNLOAD FIX:
  Colab's default HF download times out on ByT5-Large (4.7GB).
  Solution: `hf_transfer` (Rust-based parallel downloader, ~10x faster)
  + download to Google Drive first (persistent cache, survives runtime restarts)

DIFFERENCES FROM T5-Large v3:
  - max_length: 256 (vs 64) — byte-level needs more tokens
...
# ═══════════════════════════════════════════════════════════════════════
# CELL 1: SETUP + ROBUST MODEL DOWNLOAD
# ═══════════════════════════════════════════════════════════════════════
...
subprocess.check_call([sys.executable, "-m", "pip", "install", "-q",
    ...
    "sentencepiece>=0.2.0",
    "protobuf>=3.20.0",
    "hf_transfer",          # Rust-based fast downloader — fixes timeout!
])

# Enable hf_transfer globally — this is the key fix
os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "1"
print("hf_transfer enabled (10x faster HuggingFace downloads)")
...
# Persistent model cache on Drive — survives runtime restarts
MODEL_CACHE = DRIVE_BASE / 'hf_model_cache'

for d in [OUTPUT_DIR, LOG_DIR, MODEL_CACHE]:
...
assert train_file.exists(), (
    f"Train data not found: {train_file}\n"
...
# ── Download ByT5-Large to Drive (persistent, with retry) ──
MODEL_NAME = "google/byt5-large"
LOCAL_MODEL = MODEL_CACHE / "byt5-large"

def download_model_to_drive():
    """Download ByT5-Large to Google Drive with hf_transfer + retry."""
    from huggingface_hub import snapshot_download
    import time

    if (LOCAL_MODEL / "config.json").exists():
        print(f"\nByT5-Large already cached at {LOCAL_MODEL}")
        return str(LOCAL_MODEL)

    print(f"\nDownloading ByT5-Large (~4.7 GB) to Drive...")
    print("Using hf_transfer for parallel download — should take 2-5 minutes.\n")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"\nDownload complete in {elapsed:.0f}s -> {path}")
            return path
            ...
            if attempt < max_retries:
                ...
                raise RuntimeError(
                    f"Failed to download {MODEL_NAME} after {max_retries} attempts.\n"
                    "Try: (1) restart runtime and re-run, or "
                    "(2) manually download from https://huggingface.co/google/byt5-large"
                ) from e
...
for f in required_files:
    fpath = LOCAL_MODEL / f
    assert fpath.exists(), f"Missing {f} in download — incomplete. Delete {LOCAL_MODEL} and re-run."
print("Download integrity OK.")
...
#
# max_input_length: 256 (vs T5's 64)
#   ByT5 tokenizes at byte level: "wireless keyboard" = 17 bytes
#   vs ~4 subword tokens in T5. Queries up to ~60 chars need
#   256 byte-tokens including the "correct: " prefix (9 bytes).
#   With prefix: "correct: " (9) + query (max ~60) = ~70 bytes.
#   256 gives plenty of headroom for longer queries.
#
...
CONFIG = {
    ...
    "max_input_length":  256,       # byte-level needs more!
    "max_target_length": 256,
    ...
    "learning_rate":     1e-5,      # lower than T5's 3e-5
    "warmup_ratio":      0.10,
    "weight_decay":      0.01,
    "label_smoothing":   0.1,
    "max_grad_norm":     1.0,
    ...
    "save_steps":        500,
    "logging_steps":     50,
    "save_total_limit":  2,         # fewer checkpoints (model is huge)
...
print("Metrics: sentence_accuracy, token_accuracy, cer")
...
if state_files:
    ...
    ax.set_title('CER (lower=better)'); ax.grid(True, alpha=0.3)
